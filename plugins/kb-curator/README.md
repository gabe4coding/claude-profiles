...
When this plugin is loaded, a SessionStart hook (`scripts/session-start.py`) checks for a curated `.kb/` for the current repo. If `.kb/INDEX.md` exists, the hook injects an `additionalContext` block telling the assistant that a KB exists, plus the top-level INDEX and a head of the tag index. The assistant can then consult `.kb/` for context the codebase itself cannot tell. The hook is **conditional and silent**: no KB → no injection, no noise.

## Tag hygiene

`.kb/INDEX-by-tag.md` is regenerated by the Stop hook alongside the bucket indexes. It groups every entry by the values of its frontmatter `tags:` array (inline `[a, b, c]` form). The `kb` skill is instructed to consult it before assigning tags to new entries — drift between `kb-curator` / `kbcurator` / `kb_curator` makes the index useless. Aim for 2–4 tags per entry: one bucket-domain tag, one technical surface, 0–2 cross-cutting concerns.
...
- `scripts/kb-index.sh` — Stop hook that regenerates `.kb/INDEX.md` + `INDEX-by-tag.md`.
- `scripts/session-start.py` — SessionStart hook that surfaces the KB to the assistant.
- `hooks/hooks.json` — registers the Stop + SessionStart hooks.
