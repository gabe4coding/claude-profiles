...
Claude Code v2.1.147 introduced an explicit shedding hierarchy: pinned sessions (Ctrl+T in `claude agents`) survive memory pressure; non-pinned bg sessions are terminated first. Delegate sessions are non-pinned. When a delegate is shed, `cmdDelegateBgWatcher` never sees a terminal state in `state.json` and eventually times out, writing `dispatch-error.md`. The watcher timeout error message now explicitly mentions memory pressure as a possible cause. No `--pin` or `--priority` flag exists in the `claude --bg` CLI as of v2.1.150 — programmatic pinning is not available. If a pin flag becomes available in a future release, add it to `cmdDelegateBgDispatch` alongside `--permission-mode`. Until then, this is a known operational constraint on memory-constrained systems.
