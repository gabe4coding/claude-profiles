...
Claude Code v2.1.147 introduced a shedding hierarchy: pinned sessions (Ctrl+T in `claude agents`) survive memory pressure; non-pinned sessions — including every `claude-profiles` delegate — are terminated first. The watcher handles a shed session identically to a timeout: it writes `dispatch-error.md` after `bgWatcherAbandonAfter` (30 min). No programmatic pinning flag is currently available in the `claude --bg` CLI; if one ships in a future release, add it to `cmdDelegateBgDispatch` alongside `--permission-mode`. Run `claude-profiles doctor` to verify your Claude Code version is not in the known-bad list (v2.1.147 also has a Bash tool regression).
