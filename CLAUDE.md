...
**`ProfilePrefs.SubagentModel` lives only in prefs, not in `profile.json`.**
JSON tag is `subagent_model` (no leading underscore, deliberately — it's a prefs-only field, not a profile-content field). The hub's edit TUI does not expose it; users hand-edit `~/.claude-profiles/profile-prefs.json`. The dispatcher (`cmdDelegateBgDispatch`) injects `CLAUDE_CODE_SUBAGENT_MODEL=<value>` into the bg subprocess env, which pins the model used by *subagents the delegate itself spawns* — orthogonal to `_settings.model`, which pins the delegate's own model. Silently a no-op on Claude Code < v2.1.146 (the env var didn't propagate to child processes before that fix). Round-tripped through `saveProfileAt` via the `existingPrefs.SubagentModel` preservation pattern used for `Disabled`; adding a sibling prefs field without that pattern silently drops user state on profile edit.
