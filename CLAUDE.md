...
Claude Code v2.1.147 introduced an explicit shedding hierarchy: pinned bg sessions (Ctrl+T in `claude agents`) survive memory pressure; **non-pinned sessions — including all claude-profiles delegates — are shed first**. On constrained systems, a delegate that is shed while running appears identical to a timeout: `cmdDelegateBgWatcher` never sees a terminal state in `state.json`, times out, and writes `dispatch-error.md`. The parent's next prompt then injects a timeout error with no indication that memory pressure was the root cause. There is currently no `claude --bg` flag to launch a session as pinned; if one becomes available, add it to `cmdDelegateBgDispatch` alongside `--permission-mode auto`. The `dispatch-error.md` message (written by the watcher on timeout) mentions memory pressure as a candidate cause.
